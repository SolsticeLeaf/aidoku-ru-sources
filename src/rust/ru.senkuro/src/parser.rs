...
pub fn parse_listing(html: &WNode, listing: Listing) -> Option<Vec<Manga>> {
	...
	let mangas = sidebar_node
		...
		.filter_map(|manga_node| {
			...
			let nsfw = match thumb_node.select_one("span") {
				Some(span_node) => {
					if span_node.text().contains("18+") || span_node.text().contains("Explicit") {
...
pub fn parse_search_results(html: &WNode) -> Option<Vec<Manga>> {
	...
	let mangas = list_node
		...
		.filter_map(|manga_node| {
			...
			let nsfw = if categories.iter().any(|c| c.contains("18+") || c.contains("Explicit")) {
				MangaContentRating::Nsfw
			} else {
				MangaContentRating::Suggestive
			};
